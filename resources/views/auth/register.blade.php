    ...
    <div class="container">
        <div class="card register-card shadow-sm">
            <div class="card-body p-4">
                ...
                <form method="POST" action="{{ route('register') }}" x-data="{ citizenship: '{{ old('citizenship') }}', identityTypeKtp: 'KTP', identityTypePassport: 'Pasport' }">
                    @csrf

                    {{-- Input Nama (Editable, pre-fill dari Google jika ada) --}}
                    <div class="mb-3">
                        <label for="name" class="form-label">Name</label>
                        {{-- Cek $googleInfo sebelum akses key --}}
                        <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror"
                            value="{{ old('name', isset($googleInfo['name']) ? $googleInfo['name'] : '') }}" required autofocus>
                        @error('name') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>

                    {{-- Input Email (Readonly jika dari Google) --}}
                    <div class="mb-3">
                        <label for="email" class="form-label">Email Address</label>
                        {{-- Cek $googleInfo sebelum akses key dan set readonly --}}
                        <input type="email" name="email" id="email" class="form-control @error('email') is-invalid @enderror"
                            value="{{ old('email', isset($googleInfo['email']) ? $googleInfo['email'] : '') }}" required {{ isset($googleInfo) ? 'readonly' : '' }}>
                        {{-- Input hidden untuk memastikan email terkirim jika readonly --}}
                        @if(isset($googleInfo['email']))
                            <input type="hidden" name="email" value="{{ $googleInfo['email'] }}">
                        @endif
                    ...
                    {{-- Input Password --}}
                    {{-- Input Password (HANYA jika BUKAN dari Google) --}}
                    {{-- @unless anstatt @if --}}
                    @unless(isset($googleInfo))
                        <div class="mb-3">
                    ...
                    <div class="d-grid mb-3">
                        <button type="submit" class="btn btn-primary">
                            {{ isset($googleInfo) ? 'Complete Registration' : 'Register' }}
                        </button>
                    </div>

                    {{-- ... (Tombol Google Sign-In / Link Login) ... --}}
                    {{-- Tampilkan pemisah dan tombol Google HANYA jika ini BUKAN penyelesaian registrasi Google --}}
                    @unless(isset($googleInfo))
                    ...
                    <div class="text-center mt-3"> {{-- Beri sedikit margin atas jika tombol Google disembunyikan --}}
